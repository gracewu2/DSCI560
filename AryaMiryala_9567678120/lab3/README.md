...
* [cite_start]**Architectural Analysis:** Developed a baseline triple-nested loop implementation in C to establish CPU performance metrics[cite: 12].
* [cite_start]**Parallel Kernel Design:** Engineered a naïve CUDA kernel to map matrix operations across thousands of GPU threads simultaneously[cite: 52, 53].
* [cite_start]**Shared Memory Optimization:** Implemented **Shared Memory Tiling** to minimize global memory latency, significantly increasing memory throughput[cite: 89, 90].
* [cite_start]**Library Engineering:** Built a shared library (`.so`) using `nvcc` with a C-style interface, enabling seamless integration with Python's `ctypes`[cite: 153, 154].
* [cite_start]**Industry Standards:** Benchmarked hand-written kernels against **cuBLAS**, NVIDIA's premier linear algebra library, to analyze optimization gaps[cite: 136, 137].
...
### Technology Stack
* [cite_start]**Low-Level:** C, CUDA C++ [cite: 13, 51]
* [cite_start]**High-Level:** Python, NumPy [cite: 231, 232]
* [cite_start]**Interface:** Ctypes (Shared Libraries) [cite: 155]
* [cite_start]**Tooling:** NVCC Compiler, Google Cloud Platform / Colab [cite: 75, 84]
